...
{
    [Serializable]
    public class TC3ReadSymbolException : InvalidOperationException
    {
        public const string DefaultMessage = "Error writing TwinCAT 3 symbols.";

        public TC3ReadSymbolException(string contextName, TC3VariableBlock tc3VariableBlock)
            : base(DefaultMessage)
        {
            InjectData(contextName, tc3VariableBlock);
        }

        public TC3ReadSymbolException(string contextName, ResultSumValues resultSumValues)
           : base(DefaultMessage)
        {
            InjectData(contextName, resultSumValues);
        }

        public TC3ReadSymbolException(string contextName, TC3VariableBlock tc3VariableBlock, Exception inner)
        ...
        private void InjectData(string contextName, ResultSumValues resultSumValues)
        {
            Data.Add("ContextName", contextName);

            if (resultSumValues == null)
            {
                Data.Add("TC3_ResultSumCommand_NULL", true);
                return;
            }

            Data.Add("TC3_ADS_ErrorCode", resultSumValues.ErrorCode.ToString());
            Data.Add("TC3_ADS_FirstSubErrorCode", resultSumValues.FirstSubError.ToString());
            Data.Add("TC3_ADS_OverallErrorCode", resultSumValues.OverallError.ToString());

            foreach (var subError in resultSumValues.SubErrors)
